...
computed:{
  ...
  filterByLeague(){
    if (this.filter === "") {
      ...
      return this.LeaguesTheGym.filter(league => league.name.toLowerCase().includes(this.filter.toLowerCase()));
    }
  },

},
created() {
    this.league_service.findAllLeaguesByClimbingGymId(this.climbingGymId).then(response => {
  ...
  this.league_service.getAllRequests().then(response => {
    ...
    this.league_service.findRequestsByScalerId(this.localTopWay.state.userInfo.id).then(response => {
