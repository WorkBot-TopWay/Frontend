...
export default {
  ...
  methods: {
    findIndexById(id) {
      return this.climbing_gyms.findIndex(
        (climbing_gym) => climbing_gym.id === id
      );
    },
    saveClimbingGym() {
      if (this.climbing_gyms.id) {
        this.climbing_gym_Service
          .update(this.climbing_gym.id, this.climbing_gym)
          .then((response) => {
  ...
  computed: {
    ...
    createClimbingGym() {
      this.climbing_gym_Service.create(this.climbing_gym).then((response) => {
        this.climbing_gyms.push(this.climbing_gym);
        this.$toast.add({
          severity: "success",
          summary: "Successful",
