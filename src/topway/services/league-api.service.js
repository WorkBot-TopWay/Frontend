...
export class LeagueApiService {
  ...
  /////////Climbers_league/////////////
  findClimbersByClimbingGymIdAndScalerIdAndLeagueId(climbingGymId, scalerId, leagueId) {
    return http.get(`/climber-leagues?climbinggymId=${climbingGymId}&scalerId=${scalerId}&leagueId=${leagueId}`);
  }
  ...
  findRequestsLeagueByScalerId(scalerId) {
    return http.get(`/requests?scalerId=${scalerId}`);
  }

  deleteRequestByScalerIdAndLeagueId(scalerId, leagueId) {
    return http.delete(`/requests?leagueId=${leagueId}&scalerId=${scalerId}`);
  }

  createRequestByScalerIdAndLeagueId(scalerId, leagueId, data) {
    return http.post(`/requests?leagueId=${leagueId}&scalerId=${scalerId}`, data);
  }
}
